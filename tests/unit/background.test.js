  ...
  const chrome = {
    ...
    webNavigation: {
      async getAllFrames({ tabId }) {
        const frames = frameIdsByTab.get(tabId) || [0];
        return frames.map((frame) => {
          if (typeof frame === "object" && frame) {
            return {
...
test("continueMacroRun blocks archive receipt flow before approval-line assignment when the document is not accounting voucher", async () => {
  ...
  assert.equal(
    ...
    "오류: 문서 제목이 '회계전표'로 확인되지 않아 결재라인지정 전에 매크로를 중단했습니다."
  );
  assert.equal(
    harness.storage.macroRunTraceLogs.some((entry) => entry.eventType === "archive-receipt-guard-blocked"),
    true
  );
});

test("continueMacroRun allows archive receipt flow to continue for accounting voucher headings", async () => {
  const harness = loadBackgroundHarness();
  let runSingleStepCalls = 0;
...
test("continueMacroRun skips archive receipt guard for unrelated clicks", async () => {
  const harness = loadBackgroundHarness();
  let archiveChecks = 0;
  let runSingleStepCalls = 0;
...
test("continueMacroRun uses descendant frame hints when the selector lives inside an iframe-owned page", async () => {
  const harness = loadBackgroundHarness();
  const runStepFrames = [];
...
test("continueMacroRun falls back to the only direct child frame when the top frame has no selector hints", async () => {
  const harness = loadBackgroundHarness();
  const runStepFrames = [];
      ...
      return {
        ...
        detail: {
          locationHref:
            frameId === 1
              ? "https://example.com/ea/neos/edoc/delivery/receive/board/common/ReceiveBoardCommonList.do"
              : "https://example.com/gw/bizbox.do",
