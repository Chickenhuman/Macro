...
- `결재라인지정` 직전 문서 제목이 `회계전표`인지 검증
- `결재라인지정` 직전 비재무기획팀 문서의 `전표확인` 담당자 존재 여부 검증
- 위험한 결재/반영 클릭 직전 `마감부서`에 `조경환`만 단독 감지되면 자동 중단
- `전결문서` 허용 체크박스
...
4. step 목록 확인
5. 필요하면 반복 횟수/반복 간격, `전결문서`, `반복 실행 중 오류 시 다음 회차로 넘어가기`, `매크로 실행중 숨기기` 토글 설정
6. `지금 실행`
7. 실행 중 문제가 나면 `실행 중지`

`결재라인지정` 직전에는 현재 문서 제목이 `회계전표`인지 먼저 확인합니다. 제목이 `회 계 전 표`처럼 띄어져 보여도 같은 문서로 인식하며, `회계전표`로 확인되지 않으면 즉시 실행을 중단하고 팝업 상단에 경고를 표시합니다.

회계전표로 확인된 문서는 이어서 `작성부서`와 `전표확인`도 검증합니다. `작성부서`가 `재무기획팀`이 아닌데 `전표확인` 칸 옆에 담당자 이름이 없으면, 검토되지 않은 문서로 보고 즉시 실행을 중단하고 팝업 상단에 경고를 표시합니다. `작 성 부 서`, `전 표 확 인`처럼 띄어져 보여도 같은 항목으로 인식합니다.

`전결문서`를 체크하지 않은 상태에서는, `결재`/`반영` 계열 위험 클릭 직전에 현재 문서의 `마감부서` 줄 또는 가장 가까운 블록에서 이름이 `조경환` 하나만 단독으로 감지되면 즉시 실행을 중단합니다. `마감부서`가 세로쓰기처럼 `마 감 부 서`로 나뉘어 보여도 같은 항목으로 인식합니다.

### 저장 매크로
1. 현재 step을 원하는 상태로 만든 뒤 매크로 이름 입력
2. 필요하면 현재 작업의 `전결문서` 체크 상태를 맞춤
3. `현재 매크로 저장`
4. 팝업을 다시 열면 작업 step은 기본적으로 비어 있으니, 상단 저장 목록에서 선택 후 `선택 매크로 불러오기`
5. 불러오면 저장 당시의 `전결문서` 체크 상태도 함께 복원됨
6. 필요 없으면 `선택 매크로 삭제`

`선택 매크로 삭제`는 바로 삭제되지 않고, 삭제 전 확인창이 한 번 더 표시됩니다.
...
- `전결문서`를 체크하지 않았고 위험 클릭 직전 `마감부서` 블록에서 이름이 `조경환` 하나만 감지되면, 실제 클릭을 보내기 전에 실행을 멈춥니다
- 편철접수 흐름에서 `결재라인지정` 클릭 전 문서 제목이 `회계전표`로 확인되지 않으면, 실제 클릭을 보내기 전에 실행을 멈추고 팝업 상단에 경고를 표시합니다
- 편철접수 흐름에서 `작성부서`가 `재무기획팀`이 아닌데 `전표확인` 담당자가 비어 있으면, 실제 클릭을 보내기 전에 실행을 멈추고 팝업 상단에 경고를 표시합니다
- `매크로 실행중 숨기기`를 켜면 페이지 우상단의 진행 배지를 그리지 않고 step만 조용히 실행합니다
- `실행 추적 중지`를 누르면 실행 추적 로그 적재 자체를 멈추고, 다시 누르면 `실행 추적 시작`으로 재개할 수 있습니다
- 팝업의 저장 매크로, 실행/기록 제어, 빠른 추가, step 목록, JSON, 결과, 실행 추적, 디버그 영역은 각각 접고 펼칠 수 있고 마지막 상태를 기억합니다
