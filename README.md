...
- 기록된 step 순차 실행
- 반복 실행
- 반복 실행 중 오류 시 현재 회차를 종료하고 다음 회차로 진행하는 토글
...
1. 시작 페이지 열기
2. 팝업 열기
3. 저장 매크로가 필요하면 상단 저장 목록에서 선택 후 `선택 매크로 불러오기`
4. step 목록 확인
5. 필요하면 반복 횟수/반복 간격, `반복 실행 중 오류 시 다음 회차로 넘어가기`, `매크로 실행중 숨기기` 토글 설정
6. `지금 실행`
7. 실행 중 문제가 나면 `실행 중지`
...
2. `현재 매크로 저장`
3. 팝업을 다시 열면 작업 step은 기본적으로 비어 있으니, 상단 저장 목록에서 선택 후 `선택 매크로 불러오기`
4. 필요 없으면 `선택 매크로 삭제`
...
- 반복 실행에서 `반복 실행 중 오류 시 다음 회차로 넘어가기` 토글을 켜면, 실패한 현재 회차를 종료 처리하고 남은 회차를 계속 실행합니다
