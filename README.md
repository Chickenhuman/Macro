...
- 위험한 결재/반영 클릭 직전 `마감부서` 줄의 `전결` 감지 시 자동 중단
- `전결문서` 허용 체크박스
...
`전결문서`를 체크하지 않은 상태에서는, `결재`/`반영` 계열 위험 클릭 직전에 현재 문서의 `마감부서` 줄 또는 가장 가까운 블록에서 `전결`이 감지되면 즉시 실행을 중단합니다.
...
- `전결문서`를 체크하지 않았고 위험 클릭 직전 `마감부서` 줄에 `전결`이 보이면, 실제 클릭을 보내기 전에 실행을 멈춥니다
